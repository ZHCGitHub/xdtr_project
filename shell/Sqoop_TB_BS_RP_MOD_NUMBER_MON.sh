...
hadoop fs -test -e /apps/hive/warehouse/tb_bs_rp_mod_number_mon/year=${date_year}/mon=${date_month_int}
if [ $? -eq 0 ]
then
	sqoop export --connect "jdbc:mysql://${mysql_rp_ip}:3306/${mysql_rp_database}?useUnicode=true&characterEncoding=utf-8" --username ${mysql_rp_user} --password ${mysql_rp_passWord} --update-key "DEL_DATE,REG_ID,OPERATE_NAME" --update-mode allowinsert -m 3 --fields-terminated-by '${delimiter}' --table tb_bs_rp_mod_number_mon --export-dir /apps/hive/warehouse/tb_bs_rp_mod_number_mon/year=${date_year}/mon=${date_month_int}/
